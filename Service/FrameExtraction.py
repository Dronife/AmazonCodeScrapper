...
class FrameExtraction():
    ...
    def print_progress(self, iteration, total, prefix='', suffix='', decimals=3, bar_length=100):
        """
    ...
    def extract_frames(self, video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
        """
    ...
    def video_to_frames(self, video_path, frames_dir, overwrite=False, every=1, chunk_size=1000):
        """
        ...
        total = 700000  # get its total frame count
        ...
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            ...
            futures = [executor.submit(self.extract_frames, video_path, frames_dir, overwrite, f[0], f[1], every)
                        for f in frame_chunks]  # submit the processes: extract_frames(...)

            for i, f in enumerate(as_completed(futures)):  # as each process completes
                self.print_progress(i, len(frame_chunks)-1, prefix=prefix_str, suffix='Complete')  # print it's progress
